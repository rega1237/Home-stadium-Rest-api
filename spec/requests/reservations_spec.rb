...
RSpec.describe 'Reservations', type: :request do
  ...
  describe 'Reservations`s Actions' do
    ...
    it 'Return Show success' do
      get user_reservation_path(@user.id, @reservation.id), headers: { 'Content-Type': 'application/json', 'Authorization': @token }
      expect(response).to have_http_status(:success)
    end

    it 'Return Create success' do
      post user_reservations_path(@user.id), headers: { 'Content-Type': 'application/json', 'Authorization': @token }, params: {
        reservation: {
          game_id: @game.id,
